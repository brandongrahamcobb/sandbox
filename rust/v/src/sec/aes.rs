...
impl AES {
    pub fn new(iv: &Vec<u8>, game_version: i16) -> AES {
        let game_version: i16 =
            ((game_version >> 8) & 0xFF) | ((((game_version as u32) << 8) & 0xFF00) as i16);
        let iv = iv.clone();
        AES { iv, game_version }
    }
    ...
    pub fn check_header(&self, packet: &[u8]) -> bool {
        ((packet[0] ^ self.iv[2]) & 0xFF) == ((self.game_version >> 8) as u8 & 0xFF)
            && ((packet[1] ^ self.iv[3]) & 0xFF) == (self.game_version & 0xFF) as u8
    }
