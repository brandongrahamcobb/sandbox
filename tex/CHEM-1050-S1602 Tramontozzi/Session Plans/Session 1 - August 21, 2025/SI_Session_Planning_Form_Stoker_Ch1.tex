...
\textbf{Solution:} \\[0.5em]
\begin{tikzpicture}[baseline=(current bounding box.center), node distance=4cm, auto]
    % Main process node
    ...
    % Exit node for better arrow termination
    \node[draw=green!70!black, thick, rectangle, minimum width=2.5cm, minimum height=1cm, align=center, fill=green!15, right=3.5cm of decision] (exit) {Exit};
    
    % Arrows with improved routing
    \draw[-Stealth, thick, purple] (process) -- (decision);
    
    % Loop back arrow with better curve and positioning
    \draw[-Stealth, thick, red] (decision.west) -- ++(-1.5,0) |- (process.west) node[midway, left, font=\sffamily\small, xshift=-0.3cm] {No};
    
    % Exit arrow
    \draw[-Stealth, thick, green!70!black] (decision.east) -- (exit.west) node[midway, above, font=\sffamily\small] {Yes};
